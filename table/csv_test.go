...
func TestCsvToJsonQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Custom schema with column selection",
			args: args{
				sourceFile: "data.csv",
				destFile:   "output.jsonl",
				opts: []CsvToJsonOpts{
					WithCsvSchema(&schema.RowSchema{
						Columns: []*schema.ColumnSchema{
							{SourceName: "col1", ColumnName: "column1"},
							{SourceName: "col2", ColumnName: "column2"},
						},
					}),
				},
			},
			want: "COPY (SELECT col1 AS column1, col2 AS column2 FROM read_csv('data.csv', DELIM ',', HEADER TRUE)) TO 'output.jsonl' (FORMAT JSON) RETURNING COUNT(*) AS row_count;",
		},
		{
